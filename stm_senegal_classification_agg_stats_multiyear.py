...
def process_chunk(h_start, h_end, w_start, w_end):
    ...
    # Load data for feature extraction (all bands)
    bands_all = np.concatenate([
        bands_file_0[h_start:h_end, w_start:w_end, :],
        bands_file_0[h_start:h_end, w_start:w_end, :],
        bands_file_0[h_start:h_end, w_start:w_end, :],
        bands_file_0[h_start:h_end, w_start:w_end, :],
        bands_file_0[h_start:h_end, w_start:w_end, :],
    ...
    # Filter valid data
    valid_mask = np.isin(y_chunk, list(valid_classes))
    X_chunk, y_chunk = X_chunk[valid_mask], y_chunk[valid_mask]
...
# Final dataset (all in one group)
...
class_report_filename = f'/maps/mcl66/senegal/classification_reports/senegal_stm_classification_report_{mixedyear}_agg_{CLASSIFICATION}.csv'
