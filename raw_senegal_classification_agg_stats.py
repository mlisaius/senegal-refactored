...
seeds = list(range(50, 51 + NUM_SEEDS))
...
for seed in seeds: 
    ...
    def process_chunk(h_start, h_end, w_start, w_end, file_path):
        ...
        # def check_split_leakage(field_ids, train_mask, val_mask, test_mask):
        #     """
        #     Check whether any field IDs appear in more than one split.
        #     Prints summary and returns a dict with overlaps.
        #     """

        #     train_fids = set(field_ids[train_mask])
        #     val_fids   = set(field_ids[val_mask])
        #     test_fids  = set(field_ids[test_mask])

        #     overlap_train_val  = train_fids & val_fids
        #     overlap_train_test = train_fids & test_fids
        #     overlap_val_test   = val_fids & test_fids

        #     print("Unique fields per split:")
        #     print(f"  Train: {len(train_fids)}")
        #     print(f"  Val:   {len(val_fids)}")
        #     print(f"  Test:  {len(test_fids)}")
        #     print()
        ...
        #         print("⚠️ Field IDs in both Train and Val:", list(overlap_train_val)[:20], "...")
        #     if overlap_train_test:
        #         print("⚠️ Field IDs in both Train and Test:", list(overlap_train_test)[:20], "...")
        #     if overlap_val_test:
        #         print("⚠️ Field IDs in both Val and Test:", list(overlap_val_test)[:20], "...")
            
        #     if not (overlap_train_val or overlap_train_test or overlap_val_test):
        #         print("✅ No leakage detected (splits are field-disjoint).")
        ...
        # overlaps = check_split_leakage(field_ids, train_mask, val_mask, test_mask)
    ...
    print("At time of classification report generation:")
    ...
    class_report_filename = f'/maps/mcl66/senegal/classification_reports/senegal_raw_classification_report_{year}_agg_{CLASSIFICATION}.csv'
    ...
    if SAVE == "yes":
        ...
        plot_classification_map(
            ...
            cmap, 
            class_names, 
            f"/maps/mcl66/senegal/landcoverclassification/senegal_raw_prediction_map_whole_{year}_15agg.png"
        )
        # Save the prediction map as a numpy file
        np.save(f"/maps/mcl66/senegal/landcoverclassification/senegal_raw_prediction_map_whole_{year}_15agg.npy", pred_map_whole)

        # Convert the prediction map to TIFF format
        output_path = f"/maps/mcl66/senegal/landcoverclassification/senegal_raw_prediction_map_whole_{year}_15agg.tiff"
...
df_aggregate.to_csv(f"/maps/mcl66/senegal/{outfolder}/aggregate_confusion_matrix_raw_{year}_{CLASSIFICATION}.csv")
df_normalized.to_csv(f"/maps/mcl66/senegal/{outfolder}/average_normalized_confusion_matrix_raw_{year}_{CLASSIFICATION}.csv")
