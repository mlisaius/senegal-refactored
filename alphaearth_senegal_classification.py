...
TRAINING_RATIO = 0.7
MODELS = ["LogisticRegression", "RandomForest", "MLP", "RandomForest", "XBGOOST"]  # Options: "LogisticRegression", "RandomForest", or "MLP", "XGBOOST", "SVM"
NUM_SEEDS = 10  # Number of seeds for reproducibility
    ...
    for model in MODELS: #["MLP"] #, "RandomForest", "XGBOOST"]:
        ...
        for seed in seeds:
            ...
            def save_confusion_matrix(y_true, y_pred, class_names):
                ...
                # Save to CSV
                df_cm.to_csv(f"/maps/mcl66/senegal/{outfolder}/senegal_efm_classification_confmatrix_{year}{remapped_labels}_{MODEL.lower()}_{CLASSIFICATION.lower()}.csv", index=True)
            
            
            if REPORT == True:
                class_report_filename = f'/maps/mcl66/senegal/classification_reports_singlemodel/senegal_efm_classification_report_{year}{remapped_labels}_{MODEL.lower()}_{CLASSIFICATION.lower()}.csv'
            ...
            if SAVE == "yes":
                plot_classification_map(
                    ...
                    cmap, 
                    class_names, 
                    f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_{year}{remapped_labels}_{model_name.lower()}_{CLASSIFICATION.lower()}_{seed}.png"
                )
                # Save the prediction map as a numpy file
                np.save(f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_{year}{remapped_labels}_{model_name.lower()}_{CLASSIFICATION.lower()}_{seed}.npy", pred_map)
            ...
            if WHOLEMAP == True:
                if SAVE == "yes":
                    ...
                    plot_classification_map(
                        ...
                        cmap, 
                        class_names, 
                        f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}{remapped_labels}_{model_name.lower()}_{seed}.png"
                    )
                    # Save the prediction map as a numpy file
                    np.save(f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}{remapped_labels}_{model_name.lower()}_{seed}.npy", pred_map_whole)
                    
                    # Convert the prediction map to TIFF format
                    output_path = f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}{remapped_labels}_{model_name.lower()}_{seed}.tiff"
