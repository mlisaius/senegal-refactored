...
njobs = 12
chunk_size = 1000
bands_file_path = f"/maps/mcl66/senegal/alphaearth/efm_{year}_10m_clipped.npy"
...
for seed in seeds: 
    ...
    def process_chunk(h_start, h_end, w_start, w_end, file_path):
        ...
        tile_chunk = (np.load(file_path))[h_start:h_end, w_start:w_end, :]  # shape: (h, w, b)
    ...
    for model in ["RandomForest"]: #["XGBOOST", "MLP"]: # , "XGBOOST"]:
        MODEL = model
...
if CLASSIFICATION == "landcover":
    ...
    outfolder = "cropclassification"
    agg_pred_map_mask = np.load('/maps/mcl66/senegal/landcoverclassification/senegal_efm_croplandcombo_map.npy')
...
## ----------------- Collect all labels (no split) -----------------
logging.info("Loading field data...")
fielddata_df = pd.read_csv(updated_fielddata_path)
fielddata_df = fielddata_df[fielddata_df['Year'] == year]
print(f"Total fields in fielddata: {len(fielddata_df)}")

# Shuffle (optional, since no splitting)
fielddata_df = fielddata_df.sample(frac=1, random_state=seed).reset_index(drop=True)

# Collect all field IDs for this year
all_ids = fielddata_df['Id'].astype(int).unique()
logging.info(f"Total field IDs (all): {len(all_ids)}")

# Report pixel class distribution across all fields
all_mask = np.isin(field_ids, all_ids)
valid_label_mask = (labels >= 0)  # or >0 depending on convention
y = labels[all_mask & valid_label_mask]

print("All pixel class distribution:")
...
def process_chunk(h_start, h_end, w_start, w_end, file_path):
    ...
    # Load the chunk (shape: H, W, bands)
    tile_chunk = (np.load(file_path))[h_start:h_end, w_start:w_end, :]
    h, w, b = tile_chunk.shape
    s2_band_chunk = tile_chunk.reshape(-1, b)
    print(f"Chunk shape: {tile_chunk.shape}, reshaped to {s2_band_chunk.shape}")

    # Labels and field IDs for this chunk
    y_chunk = labels[h_start:h_end, w_start:w_end].ravel()
    fieldid_chunk = field_ids[h_start:h_end, w_start:w_end].ravel()
    ...
    # Keep only pixels belonging to our selected fields
    all_mask = np.isin(fieldid_chunk, all_ids)

    return (X_chunk[all_mask], y_chunk[all_mask])
...
# Concatenate results into a single dataset
...
logging.info(f"Final dataset shape: X={X_all.shape}, y={y_all.shape}")
...
class_report_filename = f'/maps/mcl66/senegal/classification_reports/senegal_efm_classification_report_{mixedyear}_agg_{CLASSIFICATION}.csv'

save_classification_report(y_all, y_pred, class_report_filename)


# compute confusion matrix
# cm = confusion_matrix(y_all, y_pred, labels=np.arange(n_classes))

# # add to aggregate (summed)
# aggregate_cm += cm

# # also store normalized (row-wise proportions)
# row_sums = cm.sum(axis=1, keepdims=True)
# row_sums[row_sums == 0] = 1  # avoid divide-by-zero
# normalized_cms.append(cm / row_sums)
...
if SAVE == "yes":
    ...
    if SAVE == "yes":
        ...
        plot_classification_map(
            ...
            cmap, 
            class_names, 
            f"/maps/mcl66/senegal/{outfolder}/senegal_tessera_prediction_map_whole_{mixedyear}{remapped_labels}_15agg.png"
        )
        # Save the prediction map as a numpy file
        np.save(f"/maps/mcl66/senegal/{outfolder}/senegal_tessera_prediction_map_whole_{mixedyear}{remapped_labels}_15agg.npy", pred_map_whole)
        
        # Convert the prediction map to TIFF format
        output_path = f"/maps/mcl66/senegal/{outfolder}/senegal_tessera_prediction_map_whole_{mixedyear}{remapped_labels}_15agg.tiff"
...
# avg_normalized_cm = np.mean(normalized_cms, axis=0)

# df_aggregate = pd.DataFrame(aggregate_cm, 
#                         index=class_names, 
#                         columns=class_names)
# df_normalized = pd.DataFrame(avg_normalized_cm, 
#                             index=class_names, 
#                             columns=class_names)

# df_aggregate.to_csv(f"/maps/mcl66/senegal/{outfolder}/aggregate_confusion_matrix_tessera_{mixedyear}_{CLASSIFICATION}.csv")
# df_normalized.to_csv(f"/maps/mcl66/senegal/{outfolder}/average_normalized_confusion_matrix_tessera_{mixedyear}_{CLASSIFICATION}.csv")
