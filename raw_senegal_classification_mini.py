...
sar_asc_bands_file_path = f"/maps/mcl66/senegal/d-pixel/merged_clipped/{year}_merged_sar_ascending_by_doy_clipped_dec10.npy"
sar_desc_bands_file_path = f"/maps/mcl66/senegal/d-pixel/merged_clipped/{year}_merged_sar_descending_by_doy_clipped_dec10.npy"
...
for seed in seeds:
    ...
    def save_confusion_matrix(y_true, y_pred, class_names):
        ...
        # Save to CSV
        df_cm.to_csv(f"/home/mcl66/code/senegal_code/landcoverclassification/senegal_raw_classification_confmatrix_{year}_{MODEL.lower()}_landcover.csv", index=True)
    
    
    if REPORT == True:
        class_report_filename = f'/home/mcl66/code/senegal_code/landcoverclassification/senegal_raw_classification_report_{MODEL.lower()}_landcover.csv'
    ...
    def batch_predict_chunk(h_start, h_end, w_start, w_end, wholemap=False):
        ...
        # check the data shapes of s2, sar_asc, and sar_desc
        logging.info(f"S2 data shape: {s2_data.shape}")
        logging.info(f"SAR Ascending data shape: {sar_asc_data.shape}")
        logging.info(f"SAR Descending data shape: {sar_desc_data.shape}")
        ...
        sar_band_chunk = sar_chunk.transpose(1, 2, 0, 3).reshape(-1, time_steps * bands) # (h*w, time_steps*bands)
        # Concatenate s2 and s1
        X_chunk = np.concatenate((s2_bands, sar_band_chunk), axis=1) 
    ...
    if SAVE == "yes":
        plot_classification_map(
            ...
            cmap, 
            class_names, 
            f"/home/mcl66/code/senegal_code/landcoverclassification/senegal_raw_prediction_map_{year}_{model_name.lower()}_{seed}.png"
        )
        # Save the prediction map as a numpy file
        np.save(f"/home/mcl66/code/senegal_code/landcoverclassification/senegal_raw_prediction_map_{year}_{model_name.lower()}_{seed}.npy", pred_map)
