...
# Load your arrays
rep_file_path = (f"/maps/mcl66/senegal/representations/{year}_representation_map_10m_utm28n_128bands_clipped.npy")  # shape (128, w, h)
scales_file_path = (f"/maps/mcl66/senegal/representations/{year}_representation_map_10m_utm28n_scales_clipped.npy")  # shape (128, w, h)
...
values_flat = values.reshape(128, num_pixels)
...
# Create DataFrame
df = pd.DataFrame(values_sel, columns=[f"value_{i+1}" for i in range(128)])
df.insert(0, "label", labels_sel)

# Export to CSV
df.to_csv(f"/home/mcl66/code/senegal_code/pixelcsvs/{year}_pixels_with_labels.csv", index=False)

print(f"Saved {len(df)} rows to  {year}_pixels_with_labels.csv")
