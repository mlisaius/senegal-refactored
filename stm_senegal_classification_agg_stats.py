...
for seed in seeds: 
    ...
    if SAMPLING == "bypercentage":
        ...
        for _, row in area_summary.iterrows():
            ...
            remaining_rows = remaining_rows.sample(frac=1).reset_index(drop=True)  # shuffle remaining
    ...
    class_report_filename = f'/maps/mcl66/senegal/classification_reports/senegal_stm_classification_report_{year}_agg_{CLASSIFICATION}.csv'
    ...
    if SAVE == "yes":
        ...
        if SAVE == "yes":
            ...
            plot_classification_map(
                ...
                cmap, 
                class_names, 
                f"/maps/mcl66/senegal/{outfolder}/senegal_stm_prediction_map_whole_{year}_15agg.png"
            )
            # Save the prediction map as a numpy file
            np.save(f"/maps/mcl66/senegal/{outfolder}/senegal_stm_prediction_map_whole_{year}_15agg.npy", pred_map_whole)
            
            # Convert the prediction map to TIFF format
            output_path = f"/maps/mcl66/senegal/{outfolder}/senegal_stm_prediction_map_whole_{year}_15agg.tiff"
...
avg_normalized_cms = np.mean(normalized_cms, axis=0)
df_aggregate = pd.DataFrame(aggregate_cm, 
                            index=class_names, 
                            columns=class_names)
df_normalized = pd.DataFrame(avg_normalized_cms, 
                             index=class_names, 
                             columns=class_names)

df_aggregate.to_csv(f"/maps/mcl66/senegal/{outfolder}/aggregate_confusion_matrix_stm_{year}_{CLASSIFICATION}.csv")
df_normalized.to_csv(f"/maps/mcl66/senegal/{outfolder}/average_normalized_confusion_matrix_stm_{year}_{CLASSIFICATION}.csv")
