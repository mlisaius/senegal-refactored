...
RUNS = 20 # Number of runs for ensemble methods
...
for seed in seeds: 
    ...
    def process_chunk(h_start, h_end, w_start, w_end, file_path):
        ...
        tile_chunk = (np.load(file_path))
        #print(f"Loaded tile_chunk shape: {tile_chunk.shape}")
    ...
    print(f"Saving classification report to: {class_report_filename}")
    ...
    if SAVE == "yes":
        ...
        if SAVE == "yes":
            ...
            plot_classification_map(
                ...
                cmap, 
                class_names, 
                f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}{remapped_labels}_15agg.png"
            )
            # Save the prediction map as a numpy file
            np.save(f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}{remapped_labels}_15agg.npy", pred_map_whole)
            
            # Convert the prediction map to TIFF format
            output_path = f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}{remapped_labels}_15agg.tiff"
...
# df_aggregate.to_csv(f"/maps/mcl66/senegal/{outfolder}/aggregate_confusion_matrix_tessera_{year}_{CLASSIFICATION}.csv")
# df_normalized.to_csv(f"/maps/mcl66/senegal/{outfolder}/average_normalized_confusion_matrix_tessera_{year}_{CLASSIFICATION}.csv")
