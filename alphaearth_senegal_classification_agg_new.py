...
for seed in seeds: #[2018, 2019]:
    ...
    if REPORT == True:
        class_report_filename = f'/maps/mcl66/senegal/classification_reports/senegal_efm_classification_report_{year}_agg_{CLASSIFICATION}.csv'

        save_classification_report(y_all, y_pred, class_report_filename)

    # Create prediction map
    #pred_map_whole = np.zeros_like(labels)
    #pred_map_whole = np.ones_like(labels) * -1  # Initialize with -1 for invalid pixels
    pred_map_whole = np.full(labels.shape, -1, dtype=np.int16)
    ...
    def batch_predict_chunk(h_start, h_end, w_start, w_end, wholemap=False):
        ...
        chunk_pred = np.full(chunk_labels.shape, -1, dtype=np.int16)
    ...
    print("checking if prediction map should be saved...")
    
    if SAVE == "yes":
        print(f"Unique classes BEFORE in prediction map: {np.unique(pred_map_whole, return_counts=True)}")

        pred_map_whole = pred_map_whole + 1  # Adjust for 1-based indexing if needed
        print(f"Unique classes in prediction map: {np.unique(pred_map_whole, return_counts=True)}")
        ...
        plot_classification_map(
            ...
            cmap, 
            class_names, 
            f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}_{CLASSIFICATION}{remapped_labels}_15agg.png"
        )
        # Save the prediction map as a numpy file
        np.save(f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}_{CLASSIFICATION}{remapped_labels}_15agg.npy", pred_map_whole)
        
        # Convert the prediction map to TIFF format
        output_path = f"/maps/mcl66/senegal/{outfolder}/senegal_efm_prediction_map_whole_{year}_{CLASSIFICATION}{remapped_labels}_15agg.tiff"
