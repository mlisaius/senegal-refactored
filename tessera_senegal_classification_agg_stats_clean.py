...
AUGMENT = True  # If True, apply SMOTE to balance training data
REMAP2021 = True  # If True, remap labels for 2021 to reduce pasture and natural vegetation to 0
RUNS = 5 # Number of different seeds/runs for ensemble methods
...
def train_mlp(X_train, y_train, X_val, y_val, num_classes, input_size):
    ...
    if AUGMENT == True:
        ...
        logging.info(f"Using class weights for loss: {class_weights}")
...
for seed in seeds: 
    ...
    # ----------------- Model training loop -----------------
    for model in ["XGBOOST", "MLP", "RandomForest"]: 
        MODEL = model
    ...
    class_report_filename = f'/maps/mcl66/senegal/classification_reports/senegal_tessera_classification_report_{year}_agg_{CLASSIFICATION}_test.csv'
    ...
    # # ----------------- Whole map prediction (optimized) -----------------
